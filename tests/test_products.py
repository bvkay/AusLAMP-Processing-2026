...
from auslamp_proc import agreement as AG, products as PR
...
def write_edi(tmp_path, period, z, err, station="TEST"):
    """The synthetic tensor written to an EDI through mt_metadata, as a product on disk would be."""
    import xarray as xr
...
def test_read_tf_drops_duplicate_periods(tmp_path):
    ...
    tf = PR.read_tf(write_edi(tmp_path, period, z, err, "DUP"))
...
def test_read_tf_masks_the_fill(tmp_path):
    ...
    tf = PR.read_tf(write_edi(tmp_path, period, z, err, "FILL"))
    ...
    assert np.nanmax(np.abs(tf.z)) < PR.FILL
...
def test_rho_phase_folds_the_yx_quadrant(tmp_path):
    ...
    tf = PR.read_tf(write_edi(tmp_path, period, z, err, "FOLD"))
    _r, _re, ph_xy, _pe = PR.rho_phase(tf.period, tf.z, tf.z_err, "xy")
    _r2, _re2, ph_yx, _pe2 = PR.rho_phase(tf.period, tf.z, tf.z_err, "yx")
...
def test_read_tf_gives_the_apparent_resistivity_it_was_built_from(tmp_path):
    ...
    period, z, err = synthetic(n=16, rho=100.0)
    tf = PR.read_tf(write_edi(tmp_path, period, z, err, "RHO"))
    rho, _e, _p, _pe = PR.rho_phase(tf.period, tf.z, tf.z_err, "xy")
...
def test_turn_is_a_rotation(angle):
    ...
    zt, tt = FR.turn_tensor(z, t, angle)
    for name, f in (("Zxy - Zyx", lambda a: a[:, 0, 1] - a[:, 1, 0]),
                    ("Zxx + Zyy", lambda a: a[:, 0, 0] + a[:, 1, 1]),
                    ("det Z", lambda a: a[:, 0, 0] * a[:, 1, 1] - a[:, 0, 1] * a[:, 1, 0])):
        a, b = f(z), f(zt)
        assert np.all(np.abs(b - a) <= 1e-9 * np.maximum(np.abs(a), 1e-12)), name
...
def test_on_grid_is_exact_at_shared_periods(tmp_path):
    ...
    tf = PR.read_tf(write_edi(tmp_path, period, z, err, "GRIDA"))
...
def test_per_decade_against_itself_is_one_and_zero(tmp_path):
    ...
    tf = PR.read_tf(write_edi(tmp_path, period, z, err, "SELF"))
...
def test_smoothness_finds_a_planted_jump(tmp_path):
    ...
    clean = PR.read_tf(write_edi(tmp_path, period, z, err, "CLEAN"))
    z2 = z.copy()
    z2[14, 0, 1] *= np.sqrt(3.0)
    rough = PR.read_tf(write_edi(tmp_path, period, z2, err, "ROUGH"))
